...
def fetch_data_from_api(item_id: int) -> dict:
    """[I/O Bound] Неоптимальный последовательный запрос к API."""
...
def main():
    ...
    # 1. Собираем данные (Тормозит тут: I/O Bound)
    all_data = []
    for item_id in ITEMS_TO_FETCH:
        data = fetch_data_from_api(item_id)
